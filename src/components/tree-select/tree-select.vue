<template>
<!--  @on-open-change="changeShow"  -->
  <div>    
    <i-select v-model="select" :placeholder="placeholder" on-open-change="false"  @on-open-change="changeShow" ref="select"> 
      <i-option v-show="false" v-for="item in data.list" :value="item[value]" :key="item[value]">{{ item[label] }}</i-option>
      <el-tree v-show="isShow" ref="tree" highlight-current default-expand-all :expand-on-click-node="false" :node-key="value" :props="defaultProps"
